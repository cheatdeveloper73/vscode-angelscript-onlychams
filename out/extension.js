...
// --- Global Stores ---
let apiCompletionItems = [];
let signatureMap = new Map(); // Key: ns::func, Type::Method, Type (for constructors)
// --- Helper Functions ---
...
function parseSignatureDeclaration(declaration, description) {
    ...
    try {
        ...
        apiData.objectTypes?.forEach(type => {
            if (type.name) {
                ...
                const processMember = (member, kind, memberName, typeName) => {
                ...
                type.methods?.forEach(m => {
                    ...
                    if (baseName && m.declaration)
                        processMember(m, vscode.CompletionItemKind.Method, baseName, type.name);
                });
                ...
                type.properties?.forEach(p => {
                    ...
                    if (propName)
                        processMember(p, vscode.CompletionItemKind.Property, propName, type.name);
                });
...
class AngelScriptSignatureHelpProvider {
    ...
    createSignatureHelp(signatures, activeParameter) {
        const signatureHelp = new vscode.SignatureHelp();
        signatureHelp.signatures = signatures.map(sig => {
            const signatureInfo = new vscode.SignatureInformation(sig.label, sig.documentation instanceof vscode.MarkdownString ? sig.documentation : new vscode.MarkdownString(sig.documentation || ''));
...
class AngelScriptHoverProvider {
    provideHover(document, position, token) {
        ...
        if (signatures && signatures.length > 0) {
            ...
            signatures.forEach((sig, index) => {
                ...
                if (documentation && documentation !== sig.label)
        ...
        if (matchingCompletion) {
            ...
            if (matchingCompletion.detail && matchingCompletion.detail !== `(${kindString}) ${matchingCompletion.label}`)
                hoverContent.appendMarkdown(`*${matchingCompletion.detail}*\n`); // Add detail if different
...
function activate(context) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider('angelscript', {
        provideCompletionItems(document, position, token, context) {
            ...
            const defaultItems = apiCompletionItems.filter(item => (!item.filterText?.includes('.')) || item.kind === vscode.CompletionItemKind.Constructor);
    ...
    const hoverProvider = vscode.languages.registerHoverProvider('angelscript', new AngelScriptHoverProvider());
    context.subscriptions.push(hoverProvider);
    // Register Signature Help Provider
    const signatureHelpProvider = vscode.languages.registerSignatureHelpProvider('angelscript', new AngelScriptSignatureHelpProvider(), '(', ',');
